...
function Weather(props){
    ...
    let cities = city_names.map(city => (
        <option key={city} value={city}>{city}</option>
    ))
    let getWeatherInfo = () =>{
        var e = document.getElementById("list");
        var city = e.options[e.selectedIndex].value;
        props.getWeatherInfo(city)
    }
    const {weather} = props.weatherReducer
    let temp = 0
    ...
    return(
        <div className='weather-block'>
            <div>
                <p>Выберите город:</p>
                <select className='list' id='list' onChange={() => {getWeatherInfo()}}>
                    {cities}
                </select>
            </div>
            {temp > 15 ? 
